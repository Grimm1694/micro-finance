  ...
  return (
    <div onMouseEnter={() => setActive(item)} className="relative ">
      <motion.p
        ...
        className="cursor-pointer text-white hover:opacity-[0.9] hover:text-[#34d399]" // Text color is now white
      >
        {item}
      </motion.p>
      {active !== null && (
...
export const Menu = ({
  ...
  return (
    <nav
      ...
      className="relative rounded-full border backdrop-blur border-[#2b2d2c] bg-transparent shadow-input flex justify-between items-center px-8 py-6" // Transparent background and custom border
    >
...
export const HoveredLink = ({ children, ...rest }: any) => {
  return (
    <motion.div 
      ...
      <Link
        {...rest}
        className="text-neutral-200 hover:text-[#34d399]" // Hover and text colors updated
      >
        {children}
      </Link>
    </motion.div>
